...
export default function TemplateCard({ template, onSelect }) {
  // Check if this is the free template
  ...
  return (
    <div className="relative group h-full">
      ...
      <button 
        ...
        className={`relative w-full h-full text-left bg-white rounded-2xl overflow-hidden transition-all duration-300 hover:-translate-y-1 flex flex-col ${
          isFree 
      ...
      >
        {/* Preview Image Area */}
        <div className="aspect-[4/3] bg-slate-50 relative overflow-hidden group-hover:opacity-100 shrink-0">
        ...
        <div className="p-5 flex flex-col flex-1">
          ...
          {/* CTA Button */}
          <div className={`w-full py-3 rounded-xl text-xs font-bold uppercase tracking-[0.15em] text-center transition-colors mt-auto ${
             isFree 
              ? 'bg-emerald-500 text-white group-hover:bg-emerald-600'
              : 'bg-slate-900 text-white group-hover:bg-black'
          }`}>
            {isFree ? 'Start Building' : 'Select Template'}
          </div>
        </div>
